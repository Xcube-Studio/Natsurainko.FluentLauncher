...
using Natsurainko.FluentLauncher.ViewModels.Downloads.Shaders;
using Natsurainko.FluentLauncher.XamlHelpers.Converters;
using Nrk.FluentCore.Resources;

namespace Natsurainko.FluentLauncher.Views.Downloads.Shaders;
    ...
    private void ContentFrame_Navigated(object sender, NavigationEventArgs e)
    {
        ...
        else
        {
            if (contentFrame.Content.GetType() == typeof(ResourcePage))
            {
                ...
                breadcrumbBar.SetPath($"ShadersDownload/{resourceName}");
            }
            else if (contentFrame.Content is ResourceDefaultPage)
            {
                breadcrumbBar.AddItem("ShadersDownload");
            }
        }
    }
